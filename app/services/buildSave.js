...
const debug = require('debug')('service:buildSave');
...
const save = {
  ...
  async buildSave(playerId) {
    ...
    const playerNotOwnsGeneratorJson = await dataMapper.getPlayerNotOwnsGenerator(playerId);
    ...
    for (let i = 0; i < generatorsOwned.length; i += 1) {
      ...
      // calc next cost
      generatorsOwned[i].next_cost = Math.floor(
        generatorsOwned[i].starting_cost * generatorsOwned[i].cost_factor ** generatorsOwned[i].number_owned,
      );
    ...
    const clickPercent = generatorsOwned.filter((element) => element.type === 2);
    const idleFlat = generatorsOwned.filter((element) => element.type === 3);
    const idlePercent = generatorsOwned.filter((element) => element.type === 4);
